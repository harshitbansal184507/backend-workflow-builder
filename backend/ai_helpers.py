...
def run_prompt(
    ...
    Conversation summary is injected as a system message when history was trimmed.
    """
    ...
    # Include recent history (already trimmed by state manager)
...
SUMMARY_THRESHOLD = 8   # summarise when history exceeds this many messages
KEEP_RECENT       = 4   # always keep the most recent N messages verbatim
...
def maybe_compress_history(
    ...
    """
    If history is long, summarise the older portion and return a shorter list.
    Returns (new_history, new_summary).
    """
    ...
    try:
        ...
        new_summary = existing_summary  # fail-safe: keep old summary

    return keep_part, new_summary


# ── 3. Extract structured variables ─────────────────────────────────────────
...
def extract_variables(
    ...
    """
    Given a list of variable definitions, pull values from recent conversation.
    extraction_plan items: {title, description, type}
    Returns {variable_name: value_or_null}
    """
...
# ── 4. AI-driven conditional routing ────────────────────────────────────────
...
def pick_next_node(
    ...
    """
    Given the candidate edges (each with a natural-language condition),
    return the target node ID that best matches the current context.
    Falls back to the first edge if uncertain.
    """
...
# ── 5. Detect conversation end signal ───────────────────────────────────────
...
def should_end_conversation(
    ...
    """
    Return True if the user clearly wants to end the conversation
    (goodbye, not interested, stop, etc.).
    """
    prompt = (
...
# ── 6. Validate user input against a rule ───────────────────────────────────
...
def validate_input(user_input: str, rule: str, model: str = "gpt-4o-mini") -> bool:
    ...
    try:
        ...
        return True  # fail-open so we don't block the user forever